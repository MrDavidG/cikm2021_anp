...
def generate_hessian(net, trainloader, layer_name, layer_type, n_batch_used=100, batch_size=2, stride_factor=3,
    ...
    for batch_idx, (inputs, _) in enumerate(trainloader):
        ...
        layer_input = net.layers_input[layer_name]
        ...
        if batch_idx == 0:
            ...
            # linear layer
            if layer_type == 'F':
                layer_input_np = layer_input.detach().cpu().numpy()
        ...
        if batch_idx % freq_moniter == 0:
            print('\r[%s] Now finish image No. %d / %d' % (
                datetime.now(), batch_idx * batch_size, n_batch_used * batch_size), end='')
...
def generate_hessian_inv_Woodbury_cavia(net, db_hessian, layer_name, layer_type, device, update_step, update_lr, SECOND,
                                        stride_factor=3, use_tf_backend=True, layer_kernel=3):
    ...
    net.eval()
    for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db_hessian):
        # setsz = n_way * k_shot
        task_num, setsz, c, h, w = x_spt.size()
        n_batch_used, batch_size = task_num, setsz

        x_spt, y_spt = x_spt.to(device), y_spt.to(device)

        for batch_idx in range(task_num):
            net.reset_context_params()

            for k in range(update_step):
                logits = net(x_spt[batch_idx], bn_training=True)
                loss = F.cross_entropy(logits, y_spt[batch_idx])

                grad = torch.autograd.grad(loss, net.context_params, retain_graph=SECOND, create_graph=SECOND)[0]
                net.context_params = net.context_params - update_lr * grad

            with torch.no_grad():
                net(x_spt[batch_idx], bn_training=True)
            ...
            if batch_idx == 0:
                if layer_type == 'C' or layer_type == 'R':
                    ...
                    layer_input_np = layer_input.permute(0, 2, 3, 1).detach().cpu().numpy()
                    # 这里shotcut需要怎么处理一下
                    layer_stride = 1 * stride_factor
            ...
            if layer_type == 'F':
                ...
                this_layer_input = layer_input.permute(0, 2, 3, 1).detach().cpu().numpy()
...
# 需要改变的是net inference的过程，因为这里面求得是maml的hessian
def generate_hessian_inv_Woodbury_maml_fsl(net, db_hessian, layer_name, layer_type, device, mask_grad, masks_dict,
                                           update_step, update_lr, SECOND,
                                           stride_factor=3, use_tf_backend=True, layer_kernel=3):
    hessian_inverse = None
    dataset_size = 0
    # freq_moniter = (n_batch_used * batch_size) / 50  # Total 50 times of printing information
    ...
    for step, (x_spt, y_spt, x_qry, y_qry) in enumerate(db_hessian):
        ...
        for batch_idx in range(task_num):
            ...
            with torch.no_grad():
                net(x_spt[batch_idx], vars=fast_weights)
...
def generate_hessian_inv_Woodbury(net, trainloader, layer_name, layer_type, \
    ...
    print('[%s] Now construct hessian_inv op for layer %s' % (datetime.now(), layer_name))
...
# 计算hessian不限于fsl设定
def generate_hessian_inv_Woodbury_maml_all(net, db_hessian, layer_name, layer_type, device, mask_grad, masks_dict,
                                           update_step, update_lr, SECOND,
                                           stride_factor=3, use_tf_backend=True):
    hessian_inverse = None
    dataset_size = 0
    # freq_moniter = (n_batch_used * batch_size) / 50  # Total 50 times of printing information
    ...
    net.eval()
    n_batch_used = len(db_hessian)
    batch_size = db_hessian.batch_size
    for step, (x_spt, y_spt) in enumerate(db_hessian):
        x_spt, y_spt = x_spt.to(device), y_spt.to(device)
